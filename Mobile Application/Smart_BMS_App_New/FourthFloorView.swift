...
struct FourthFloorView: View {
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 16) {
                    ...
                    LazyVGrid(columns: columns, spacing: 20) {
                        deviceCard(imageName: "air.conditioner", deviceName: "Air Condition", key: "AirCondition", destination: AnyView(AirConditionView()))
                        deviceCard(imageName: "lightbulb", deviceName: "Smart Light", key: "SmartLight", destination: AnyView(SmartLightView()))
                        deviceCard(imageName: "air.purifier", deviceName: "AirPurifier", destination: nil)
                        deviceCard(imageName: "lightbulb.fill", deviceName: "LED Light", key: "LEDLight", destination: AnyView(DigitalLEDView()))
                        deviceCard(imageName: "wind", deviceName: "HVAC", key: "HVAC", destination: nil)
                        deviceCard(imageName: "aqi.medium", deviceName: "IAQ Sensor", key: "IAQSensor", destination: AnyView(SensorDataView()))
                    }
    ...
    func sendDeviceStates() {
        for (cabin, devices) in cabinDevices {
            ref.child("Fourth Floor").child(cabin).setValue(devices)
        }
    }
    ...
    var welcomeSection: some View {
        HStack {
            VStack(alignment: .leading) {
                Text("Welcome to Floor Four!")
                    .font(.headline)
                Text("Take control as you continue your seamless journey of home automation")
                    .font(.caption)
                    .foregroundColor(.gray)
            }
            Spacer()
    ...
    var addDeviceButton: some View {
        Button(action: { print("Add device tapped") }) {
            HStack {
    ...
    func deviceCard(imageName: String, deviceName: String, key: String? = nil, destination: AnyView?) -> some View {
        let binding = Binding<Bool>(
            get: { key == nil ? false : cabinDevices[selectedCabin]?[key!] ?? false },
            set: { newValue in
                if let key = key {
                    cabinDevices[selectedCabin]?[key] = newValue
...
struct FourthFloorView_Previews: PreviewProvider {
